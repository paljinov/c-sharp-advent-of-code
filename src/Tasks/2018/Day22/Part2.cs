...
As you leave, he hands you some tools: a torch and some climbing gear. You can't
equip both tools at once, but you can choose to use neither.
...
climbing gear, torch, or neither always takes seven minutes, regardless of which
tools you start with. (For example, if you are in a rocky region, you can switch
from the torch to the climbing gear, but you cannot switch to neither.)
...
minutes. In it, 21 minutes are spent switching tools (three times, seven minutes
each) and the remaining 24 minutes are spent moving.
...
namespace App.Tasks.Year2018.Day22
{
    public class Part2 : ITask<int>
    {
        ...
        {
            int depth = scanRepository.GetDepth(input);
            (int X, int Y) targetCoordinates = scanRepository.GetTargetCoordinates(input);
            int fewestNumberOfMinutesNeededToReachTheTarget =
                cave.CalculateFewestNumberOfMinutesNeededToReachTheTarget(depth, targetCoordinates);
