...
namespace App.Tasks.Year2020.Day18
{
    public class Part2 : ITask<long>
    {
        ...
        public Part2()
        {
            expressionsRepository = new ExpressionsRepository();
            expressionsEvaluator = new ExpressionsEvaluator();
        }

        public long Solution(string input)
        {
            string[] expressions = expressionsRepository.GetExpressions(input);
            long result = expressionsEvaluator.ResultingValuesSumForAdditionBeforeMultiplication(expressions);
