...
namespace App.Tasks.Year2020.Day25
{
    public class Part1 : ITask<int>
    {
        ...
        public Part1()
        {
            publicKeyRepository = new PublicKeyRepository();
            encryptionKey = new EncryptionKey();
        }

        public int Solution(string input)
        {
            int cardPublicKey = publicKeyRepository.GetCardPublicKey(input);
            int doorPublicKey = publicKeyRepository.GetDoorPublicKey(input);

            int handshakeEncryptionKey = encryptionKey.GetHandshakeEncryptionKey(cardPublicKey, doorPublicKey);

            return handshakeEncryptionKey;
        }
    }
}
