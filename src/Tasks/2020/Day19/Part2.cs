...
namespace App.Tasks.Year2020.Day19
{
    public class Part2 : ITask<int>
    {
        ...
        private readonly ValidMessages validMessages;

        public Part2()
        {
            messagesRepository = new MessagesRepository();
            validMessages = new ValidMessages();
        }

        public int Solution(string input)
        {
            Dictionary<string, List<string>> rules = messagesRepository.GetRules(input);
            // Update rule "8" and "11"
            rules["8"] = new List<string> {
                "42",
                "42 8",
            };
            rules["11"] = new List<string> {
                "42 31",
                "42 11 31"
            };

            string[] receivedMessages = messagesRepository.GetReceivedMessages(input);

            int followRuleZero = validMessages.CountMessagesWhichMatchRuleZero(rules, receivedMessages);
