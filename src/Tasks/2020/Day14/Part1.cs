...
namespace App.Tasks.Year2020.Day14
{
    ...
    {
        private readonly InitializationProgramRepository initializationProgramRepository;

        private readonly Memory memory;

        public Part1()
        {
            initializationProgramRepository = new InitializationProgramRepository();
            memory = new Memory();
        }

        public long Solution(string input)
        {
            List<ProgramSequence> initializationProgram = initializationProgramRepository.GetInitializationProgram(input);
            long memoryValuesSum = memory.MemoryValuesSumForDecoderChipVersion1(initializationProgram);
