...
namespace App.Tasks.Year2020.Day13
{
    public class Part1 : ITask<int>
    {
        ...
        private readonly DepartureBus departureBus;

        public Part1()
        {
            notesRepository = new NotesRepository();
            departureBus = new DepartureBus();
        }

        public int Solution(string input)
        {
            int earliestDepartureTimestamp = notesRepository.GetEarliestDepartureTimestamp(input);
            List<int> busIds = notesRepository.GetBusIds(input);
            int product =
                departureBus.CalculateProductOfEarliestBusIdAndMinutesWaiting(earliestDepartureTimestamp, busIds);
