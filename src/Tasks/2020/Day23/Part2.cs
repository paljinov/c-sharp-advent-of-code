...
namespace App.Tasks.Year2020.Day23
{
    ...
    {
        private const int MOVES = 10000000;

        private const int CUPS = 1000000;
        ...
        public Part2()
        {
            cupsRepository = new CupsRepository();
            game = new Game();
        }
        ...
        public long Solution(string input)
        {
            ...
            long productOfTwoCupsClockwiseFromOne = game.ProductOfTwoCupsClockwiseFromOne(cups, MOVES);

            return productOfTwoCupsClockwiseFromOne;
        }

        private void AddCups(LinkedList<int> cups)
        {
            LinkedListNode<int> node = cups.Last;
            int n = cups.Count;
            for (int i = n + 1; i <= CUPS; i++)
            {
                cups.AddAfter(node, new LinkedListNode<int>(i));
                node = node.Next;
            }
        }
    }
