...
namespace App.Tasks.Year2020.Day20
{
    public class Part1 : ITask<long>
    {
        ...
        private readonly AssembleImage assembleImage;

        public Part1()
        {
            tilesRepository = new TilesRepository();
            assembleImage = new AssembleImage();
        }

        public long Solution(string input)
        {
            Dictionary<int, string[]> tiles = tilesRepository.GetTiles(input);
            long cornerTilesIdsProduct = assembleImage.CalculateCornerTilesIdsProduct(tiles);
