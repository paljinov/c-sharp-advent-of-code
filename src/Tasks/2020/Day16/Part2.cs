...
namespace App.Tasks.Year2020.Day16
{
    public class Part2 : ITask<long>
    {
        ...
        {
            List<Field> fields = notesRepository.GetFields(input);
            List<int> ticket = notesRepository.GetTicket(input);
            List<List<int>> nearbyTickets = notesRepository.GetNearbyTickets(input);

            long product = tickets.GetTicketDepartureFieldsProduct(fields, ticket, nearbyTickets);
