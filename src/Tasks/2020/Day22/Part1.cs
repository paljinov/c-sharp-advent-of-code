...
namespace App.Tasks.Year2020.Day22
{
    ...
    {
        private readonly PlayersStartingDecksRepository playersStartingDecksRepository;

        private readonly CardGame cardGame;

        public Part1()
        {
            playersStartingDecksRepository = new PlayersStartingDecksRepository();
            cardGame = new CardGame();
        }

        public int Solution(string input)
        {
            Queue<int> player1StartingDeck = playersStartingDecksRepository.GetPlayerStartingDeck(input, 1);
            Queue<int> player2StartingDeck = playersStartingDecksRepository.GetPlayerStartingDeck(input, 2);

            int cornerTilesIdsProduct = cardGame.GetWinningPlayerScore(player1StartingDeck, player2StartingDeck);
