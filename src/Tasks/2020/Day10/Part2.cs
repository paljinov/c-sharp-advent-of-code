...
namespace App.Tasks.Year2020.Day10
{
    public class Part2 : ITask<long>
    {
        ...
        private readonly AdaptersChain adaptersChain;

        public Part2()
        {
            adaptersRepository = new AdaptersRepository();
            adaptersChain = new AdaptersChain();
        }

        public long Solution(string input)
        {
            int[] adapters = adaptersRepository.GetAdapters(input);
            long distinctAdaptersArrangements = adaptersChain.CountDistinctAdaptersArrangements(adapters);

            return distinctAdaptersArrangements;
        }
    }
}
