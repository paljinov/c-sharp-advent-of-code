...
namespace App.Tasks.Year2017.Day13
{
    ...
    {
        private readonly FirewallLayersRepository firewallLayersRepository;

        private readonly Firewall firewall;

        public Part2()
        {
            firewallLayersRepository = new FirewallLayersRepository();
            firewall = new Firewall();
        }

        public int Solution(string input)
        {
            List<FirewallLayer> firewallLayers = firewallLayersRepository.GetFirewallLayers(input);
            int fewestNumberOfDelayedPicosecondsToAvoidBeingCaught =
                firewall.CalculateFewestNumberOfDelayedPicosecondsToAvoidBeingCaught(firewallLayers);

            return fewestNumberOfDelayedPicosecondsToAvoidBeingCaught;
        }
    }
}
