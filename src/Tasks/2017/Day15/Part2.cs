...
namespace App.Tasks.Year2017.Day15
{
    ...
    {
        private const int TOTAL_PAIRS = 5000000;

        private const int GENERATOR_A_VALUE_MULTIPLE_OF = 4;

        private const int GENERATOR_B_VALUE_MULTIPLE_OF = 8;
        ...
        public Part2()
        {
            generatorsRepository = new GeneratorsRepository();
            generatePairs = new GeneratePairs();
        }

        public int Solution(string input)
        {
            (int generatorAStartValue, int generatorBStartValue) = generatorsRepository.GetStartingValues(input);
            int judgeFinalCount = generatePairs.CalculateJudgeFinalCountForPickyCriteria(
                generatorAStartValue,
                generatorBStartValue,
                TOTAL_PAIRS,
                GENERATOR_A_VALUE_MULTIPLE_OF,
                GENERATOR_B_VALUE_MULTIPLE_OF
            );
