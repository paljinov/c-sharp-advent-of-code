...
namespace App.Tasks.Year2017.Day25
{
    public class Part1 : ITask<int>
    {
        ...
        {
            char startState = blueprintRepository.GetStartState(input);
            int steps = blueprintRepository.GetStepsAfterWhichDiagnosticChecksumIsPerformed(input);
            Dictionary<char, ConditionInstructions> instructions = blueprintRepository.GetInstructions(input);

            int diagnosticChecksum = turingMachine.CalculateDiagnosticChecksum(startState, steps, instructions);
