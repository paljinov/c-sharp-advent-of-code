...
- You can instruct a node to move (not copy) all of its data to an adjacent node
  (if the destination node has enough space to receive the data). The sending
...
namespace App.Tasks.Year2016.Day22
{
    public class Part1 : ITask<int>
    {
        ...
        {
            List<Node> nodes = nodesRepository.GetNodes(input);
            int nodesViablePairs = this.nodes.CountNodesViablePairs(nodes);
