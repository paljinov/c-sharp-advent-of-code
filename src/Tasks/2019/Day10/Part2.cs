...
namespace App.Tasks.Year2019.Day10
{
    ...
    {
        private readonly AsteroidMapRepository asteroidMapRepository;

        private readonly MonitoringStation monitoringStation;

        public Part2()
        {
            asteroidMapRepository = new AsteroidMapRepository();
            monitoringStation = new MonitoringStation();
        }

        public int Solution(string input)
        {
            bool[,] asteroidMap = asteroidMapRepository.GetAsteroidMap(input);
            int result = monitoringStation.CalculateResultForTwoHundredthAsteroidToBeVaporized(asteroidMap);
