...
namespace App.Tasks.Year2019.Day3
{
    ...
    {
        private readonly WiresPathsRepository wiresPathsRepository;

        private readonly CrossedWires crossedWires;

        public Part2()
        {
            wiresPathsRepository = new WiresPathsRepository();
            crossedWires = new CrossedWires();
        }

        public int Solution(string input)
        {
            Dictionary<int, List<Instruction>> wiresPaths = wiresPathsRepository.GetWiresPaths(input);
            int fewestCombinedSteps =
                crossedWires.CalculateFewestCombinedStepsTheWiresMustTakeToReachAnIntersection(wiresPaths);
