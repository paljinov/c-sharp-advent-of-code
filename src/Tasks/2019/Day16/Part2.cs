...
namespace App.Tasks.Year2019.Day16
{
    ...
    {
        private const int INPUT_SIGNAL_REPETITIONS = 10000;

        private const int MESSAGE_OFFSET = 7;
        ...
        public Part2()
        {
            inputSignalRepository = new InputSignalRepository();
            flawedFrequencyTransmissionAlgorithm = new FlawedFrequencyTransmissionAlgorithm();
        }

        public string Solution(string input)
        {
            int[] inputSignal = inputSignalRepository.GetInputSignal(input);
            string eightDigitMessageEmbeddedInTheFinalOutputList = flawedFrequencyTransmissionAlgorithm
                .CalculateEightDigitMessageEmbeddedInTheFinalOutputList(
                    inputSignal, INPUT_SIGNAL_REPETITIONS, MESSAGE_OFFSET);
