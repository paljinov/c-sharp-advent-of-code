...
namespace App.Tasks.Year2021.Day10
{
    ...
    {
        private readonly NavigationSubsystemRepository navigationSubsystemRepository;

        private readonly SyntaxScore syntaxScore;

        public Part2()
        {
            navigationSubsystemRepository = new NavigationSubsystemRepository();
            syntaxScore = new SyntaxScore();
        }

        public long Solution(string input)
        {
            string[] navigationSubsystem = navigationSubsystemRepository.GetNavigationSubsystem(input);
            long autocompleteToolsMiddleScore = syntaxScore.CalculateAutocompleteToolsMiddleScore(navigationSubsystem);
