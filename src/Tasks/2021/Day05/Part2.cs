...
namespace App.Tasks.Year2021.Day5
{
    ...
    {
        private readonly LineSegmentsRepository lineSegmentsRepository;

        private readonly Lines lines;

        public Part2()
        {
            lineSegmentsRepository = new LineSegmentsRepository();
            lines = new Lines();
        }

        public int Solution(string input)
        {
            LineSegment[] lineSegments = lineSegmentsRepository.GetLineSegments(input);
            int pointsWhereAtLeastTwoLinesOverlap =
                lines.CountPointsWhereAtLeastTwoLinesOverlapWhenConsideringDiagonalLines(lineSegments);
