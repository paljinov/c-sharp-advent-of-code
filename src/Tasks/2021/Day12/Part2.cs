...
namespace App.Tasks.Year2021.Day12
{
    ...
    {
        private readonly CaveSystemRepository caveSystemRepository;

        private readonly CaveSystemPaths caveSystemPaths;

        public Part2()
        {
            caveSystemRepository = new CaveSystemRepository();
            caveSystemPaths = new CaveSystemPaths();
        }

        public int Solution(string input)
        {
            (string, string)[] caveSystem = caveSystemRepository.GetCaveSystem(input);
            int caveSystemPathsThatVisitSmallCavesAtMostTwice =
                caveSystemPaths.CountCaveSystemPathsThatVisitSmallCavesAtMostTwice(caveSystem);
